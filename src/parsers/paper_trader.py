...
class PaperTrader:
    ...
    def _check_close_conditions(
        ...
        Close logic (memecoins need room to run):
        1. Rug detected → immediate close
        2. Stop loss: -50% from entry → close
        3. Take profit: >= 3x from entry → close (balanced for memecoins)
        4. Trailing stop: after hitting 2x, close if price drops 30% from max
        5. Early stop: if < -20% after 30 minutes → close early (not recovering)
        6. Timeout: 8 hours max → close
        """
        if is_rug:
            return "rug"
        ...
        if pos.entry_price and pos.entry_price > 0:
            ...
            # Hard take profit at 3x (balanced for memecoin volatility)
            if multiplier >= self._take_profit_x:
                return "take_profit"

            # Trailing stop: after 2x, protect gains — close if 30% drawdown from max
