"""Token scoring — pure function, no IO dependencies."""
...
def compute_score(
    ...
    *,
    ...
    # Phase 16 parameters
    holder_growth_pct: float | None = None,  # holder count growth % vs prev snapshot
    has_website: bool | None = None,
    domain_age_days: int | None = None,
    tg_member_count: int | None = None,
    llm_risk_score: int | None = None,  # 0-100 from LLM analysis
) -> int:
    """Compute a 0-100 score for a token snapshot (v2).
    ...
    - Security bonuses/penalties: -20 to +30 pts
    - Buy pressure bonus: 0-10 pts
    - Smart money bonus: 0-15 pts
    - Holder velocity bonus: 0-10 pts
    - Creator risk penalty: 0 to -20 pts
    - Whale dynamics: -10 to +8 pts
    - LP lock duration: 0-5 pts
    ...
    - Dev holds penalty: -15 to +3 pts (Phase 11)
    - Volatility penalty: -5 to +2 pts (Phase 11)
    - Rugcheck penalty: -15 to +5 pts (Phase 11)
    - 24h sustained interest: -5 to +3 pts (Phase 11)
    - Hard disqualifier: honeypot or jupiter_banned (instant 0)
    - is_mintable: soft penalty (-15) instead of hard 0
    - Holder growth velocity: -3 to +15 pts (Phase 16)
    - Website/domain: 0 to +3 pts (Phase 16)
    - Telegram community: 0 to +5 pts (Phase 16)
    - LLM risk: -5 to +3 pts (Phase 16)
    """
    liquidity_usd = snapshot.liquidity_usd or snapshot.dex_liquidity_usd
    if liquidity_usd is None:
        return 0  # no liquidity data = score 0, not None

    # Hard disqualifier — honeypot only (GMGN or GoPlus)
    ...
    # --- Security bonuses/penalties (-15 to +25 pts) ---
    if security is not None:
        ...
        if (
            ...
            and security.top10_holders_pct < Decimal("30")
        ):
            score += 5
        # Soft penalty for mintable tokens (not hard 0 — many fresh tokens are mintable)
    ...
    if dev_holds_pct is not None:
        if dev_holds_pct >= 80:
            ...
            score -= 8
        elif dev_holds_pct >= 35:
            score -= 5
        elif dev_holds_pct >= 25:
            score -= 1  # transition zone (not great, not terrible)
        elif 10 <= dev_holds_pct < 25:
            score += 3  # skin in the game
        elif 5 <= dev_holds_pct < 10:
            score += 1  # minimal but present
        elif dev_holds_pct < 5:
            score -= 3  # dev sold out = exit risk
    ...
    if b24 is not None and s24 is not None and s24 > 0:
        ratio_24h = b24 / s24
        ratio_1h = buy_ratio  # from above
        if ratio_24h >= 2.0 and ratio_1h is not None and ratio_1h >= 2.0:
            score += 3  # sustained buying interest
        elif ratio_24h < 0.3:
            score -= 5  # sustained selling pressure

    # --- Phase 12: PRE_SCAN risk boost (0 to -45 pts) ---
    score -= mint_risk_boost

    # --- Phase 12: Sell simulation failed (-20 pts) ---
    if sell_sim_failed:
        score -= 20

    # --- Phase 12: Bundled buy detected (-10 pts) ---
    if bundled_buy_detected:
        score -= 10

    # --- Phase 12: Pump.fun serial deployer (-5 to -15 pts) ---
    ...
    # --- Phase 12: Raydium LP verification (-2 to +3 pts) ---
    ...
    # --- Phase 13: Fee payer sybil (-8 to -15 pts) ---
    ...
    # --- Phase 13: Funding chain risk (0 to -12 pts) ---
    ...
    # --- Phase 13: Token convergence (-15 pts) ---
    ...
    # --- Phase 13: Wash trading (-10 pts) ---
    if wash_trading_suspected:
        score -= 10

    # --- Phase 13: Rugcheck danger count (-5 to -12 pts) ---
    ...
    # --- Phase 13: Bubblemaps decentralization (-8 to +3 pts) ---
    ...
    # --- Phase 13: SolSniffer cross-validation (-5 to +3 pts) ---
...
def _best_volume(snapshot: TokenSnapshot) -> float | None:
    """Return best available volume metric (prefer 1h > 24h > 5m)."""
    for vol in (snapshot.volume_1h, snapshot.volume_24h, snapshot.volume_5m):
        if vol is not None:
            return float(vol)
    for vol in (snapshot.dex_volume_1h, snapshot.dex_volume_24h, snapshot.dex_volume_5m):
        if vol is not None:
            return float(vol)
    return None
