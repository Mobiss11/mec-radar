...
class TestPhase38CompoundCopycatGuard:
    ...
    def test_compound_rugcheck_score_3000_blocked_dirty_gate(self):
        """Phase 48: MOLTGEN rc=3501 → dirty_token_gate fires before compound rules.
        All rc > 1000 are blocked by clean-only filter."""
        snapshot = _make_snapshot(holders_count=50)
        sec = _make_security(lp_burned=False, lp_locked=False)
        result = evaluate_signals(
            snapshot, sec,
    ...
    def test_compound_rugcheck_score_2500_blocked_dirty_gate(self):
        """Phase 48: rc=2500 → dirty_token_gate fires before compound evaluation."""
        snapshot = _make_snapshot(holders_count=50)
        sec = _make_security(lp_burned=False, lp_locked=False)
        result = evaluate_signals(
            snapshot, sec,
    ...
    def test_compound_rugcheck_score_5500_blocked_dirty_gate(self):
        """Phase 48: rc=5500 → dirty_token_gate fires before compound evaluation."""
        snapshot = _make_snapshot(holders_count=50)
        sec = _make_security(lp_burned=False, lp_locked=False)
        result = evaluate_signals(
            snapshot, sec,
